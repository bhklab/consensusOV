...
konecny.supplementary.data <- gdata::read.xls(system.file("extdata", "jnci_JNCI_14_0249_s05.xls", package="MetaGx"), sheet=4)
...
konecny.centroids[,2:5] <- sapply(konecny.centroids[,2:5], function(x) ave(x, konecny.centroids$EntrezGeneID, FUN=mean))
...
helland.supplementary.type.1 <- gdata::read.xls(system.file("extdata", "journal.pone.0018064.s015.XLS", package="MetaGx"), sheet=1)
helland.supplementary.type.2 <- gdata::read.xls(system.file("extdata", "journal.pone.0018064.s015.XLS", package="MetaGx"), sheet=2)
helland.supplementary.type.4 <- gdata::read.xls(system.file("extdata", "journal.pone.0018064.s015.XLS", package="MetaGx"), sheet=3)
helland.supplementary.type.5 <- gdata::read.xls(system.file("extdata", "journal.pone.0018064.s015.XLS", package="MetaGx"), sheet=4)
helland.supplementary.tables <- list(C1=helland.supplementary.type.1, C2=helland.supplementary.type.2, C4=helland.supplementary.type.4, C5=helland.supplementary.type.5)

entrez.id.logFC.list.helland <- lapply(helland.supplementary.tables, function(x) {
...
verhaak.genesets.entrez.ids <- lapply(verhaak.gene.symbols, function(gene.names) do.call(c, as.list(org.Hs.egALIAS2EG)[gene.names]))
...
esets.rescaled.classified <- lapply(esets.not.rescaled.classified, function(eset) {
  exprs(eset) <- t(scale(t(exprs(eset))))
  eset
})

esets.rescaled.classified.filteredgenes <- lapply(esets.not.rescaled.classified, function(eset) {
  eset <- eset[as.character(fData(eset)$EntrezGene.ID) %in% as.character(verhaak.entrez.ids),]
  })
